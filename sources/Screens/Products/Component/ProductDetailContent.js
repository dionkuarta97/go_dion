...
const ProductDetailContent = (props) => {
        ...
        ExpoIap.setPurchaseListener(
            async ({responseCode, results, errorCode}) => {
                ...
                if (responseCode === ExpoIap.IAPResponseCode.OK) {
                        ...
                        if (!purchase.acknowledged) {
                            ...
                            if (isLogin) {
                                dispatch(getPaymentApple(item))
                                    ...
                                    .finally(async () => {
                                        ExpoIap.finishTransactionAsync(
                                            purchase,
                                            true
                                        );
                                        await ExpoIap.disconnectAsync();
                    ...
                    responseCode === ExpoIap.IAPResponseCode.USER_CANCELED
                ) {
                    toast.show({
                        title: "Informasi",
                        status: "warning",
    ...
    console.log(
        Math.floor(moment.duration(givenAkhir?.diff(current)).asDays())
    );
                    ...
                    {infoWaktu && (
                        <DefaultModal>
                            <Center>
                                ...
                                />
                                <Text style={{textAlign: "center"}}>
                                    Setelah melewati waktu tersebut, produk ini
                                    akan dianggap hangus. Yakin mau beli ?
                                </Text>
