         ...
         fetch(urlBase + urlGroupedProduk + "?limit=5", {
            ...
            .then((json) => {
               if (json.status) {
                  ...
                  dispatch(setGroupedProduk(defaultFailedState(json.message)));
            })
            .catch((err) => {
            ...
            .then((json) => {
               ...
               if (json.status) {
                  ...
                  if (page === 1) {
                     ...
                     dispatch(
                        setAllProduk(
                           defaultDoneState([...product, ...json.data])
                        )
                     );
                  } else {
                     dispatch(setLoading(false));
         ...
         fetch(urlBase + urlGroupedProduk + `/${section_id}/purchased/count`, {
            ...
            .then((json) => {
               if (json.status) {
                  ...
                  dispatch(
                     setTotalPurchasedProduk(defaultFailedState(json.message))
                  );
            })
            .catch((err) => {
         ...
         fetch(urlBase + urlGroupedProduk + `/${produkId}/include`, {
            ...
            .then((json) => {
               if (json.status) {
                  ...
                  dispatch(setIncludesProduk(defaultFailedState(json.message)));
            })
            .catch((err) => {
   ...
   return async (dispatch, getState) => {
      ...
      try {
         ...
         if (result.status)
            ...
            dispatch(setSearchProductTitle(defaultFailedState(result.message)));
      } catch (err) {
