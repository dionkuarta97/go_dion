    ...
    <DefaultBottomSheet title="Province" onClose={() => props.onClose()}>
      ...
      {listProvince.data !== null && (
        <FlatList
          ...
          data={listProvince.data.filter((value) => value.provinsi.toLowerCase().includes(search.toLowerCase()))}
          renderItem={({ item, index }) => {
            return (
              <TouchableOpacity key={`province-${item.idprovinsi}`} onPress={() => props.onSelect(item)}>
