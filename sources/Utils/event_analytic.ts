export enum EventAnalytic {
    GoProductDetail = 'product_detail',
    GoPaketBelajarAll = 'paket_belajar_all',
    GoPaketTryoutAll = 'paket_tryout_all',
    GoPaketBukusakti = 'paket_bukusakti',
    GoBuyNow = 'buy_now',
    GoAddToCart = 'add_to_cart',
    GoProfileProdi = 'profile_prodi',
    GoProfileEdit = 'profile_edit',
    GoLeaderboard = 'leaderboard',
    GoReportTryout = 'report_tryout',
    GoReportMateri = 'report_materi',
}